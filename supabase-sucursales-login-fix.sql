-- AppSalon Pro — fix login admin_sucursal (teléfono interno + finalize)
-- Ejecutar en Supabase SQL Editor después de:
--   supabase-sucursales-setup.sql
--   supabase-sucursales-login-phone.sql
--   supabase-sucursales-admin-trigger.sql (o supabase-branch-admin-finalize.sql)

-- Finalizar perfil sucursal: metadata, código, teléfono interno o id explícito (App Salón)
...
BEGIN
  ...
  IF v_sucursal_id IS NULL THEN
    RAISE EXCEPTION 'Falta vincular la sucursal. Desde matriz: Sucursales → cerrar sesión → Activar sucursal con código y PIN.';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Esta cuenta no coincide con la sucursal indicada. Revisá código y PIN.';
  END IF;
...
-- Backfill login_phone en sucursales antiguas
UPDATE public.sucursales s
SET login_phone = public.branch_login_phone_from_codigo(s.codigo)
WHERE s.login_phone IS NULL
  AND s.codigo IS NOT NULL
  AND public.branch_login_phone_from_codigo(s.codigo) IS NOT NULL;

-- Trigger auth: teléfonos +502999… sin metadata no deben ser admin global

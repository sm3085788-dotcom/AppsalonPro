...
export function BookingForm({
  ...
  const deposit = useMemo(
    () =>
      selectedService
        ? computeBookingDepositGtq(selectedService.precio)
        : null,
    [selectedService],
  );
    ...
    <div className="grid gap-8 lg:grid-cols-[1fr_340px]">
      <div className="space-y-6 rounded-2xl border border-border bg-surface p-6">
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-gold py-3 text-sm font-semibold text-charcoal transition-colors hover:bg-gold-soft"
        >
          Reservar y pagar anticipo
          {deposit != null ? ` · ${formatQ(deposit)}` : ''}
        </button>
      </div>
      ...
      <aside className="h-fit space-y-4 rounded-2xl border border-border bg-surface p-6">
        ...
        <Row
          label={BOOKING_DEPOSIT_LABEL}
          value={deposit != null ? formatQ(deposit) : '—'}
          highlight
        />

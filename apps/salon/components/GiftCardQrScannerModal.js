...
import { parseGiftCardQrPayload } from '@appsalon/shared-config';
import { useTheme } from '../theme/ThemeProvider';
import { SalonButton } from './luxury/SalonButton';
import { modalSheetBottomPad } from './luxury';
import { useSafeAreaInsets } from 'react-native-safe-area-context';

export function GiftCardQrScannerModal({ visible, onClose, onPayload }) {
  ...
  const submitCode = (raw) => {
    const code = parseGiftCardQrPayload(raw);
    if (!code) {
      Alert.alert('Código inválido', 'Escaneá el QR de la tarjeta VIP o ingresá el código GC-…');
      return false;
    }
      ...
      <View style={styles.backdrop}>
        <View style={[styles.sheet, { backgroundColor: c.background, paddingBottom: modalSheetBottomPad(insets) }]}>
          <View style={styles.head}>
            <Text style={[styles.title, { color: c.foreground }]}>Escanear tarjeta regalo</Text>
            <TouchableOpacity onPress={onClose} hitSlop={12}>
              <X size={22} color={c.foregroundMuted} />
            </TouchableOpacity>
          </View>
          <Text style={[styles.sub, { color: c.foregroundMuted }]}>
            QR VIP · prefijo APSGIFT o código GC-XXXXXXXX
          </Text>

          {!permission?.granted ? (
            <View style={styles.permBox}>
              <Text style={[styles.sub, { color: c.foregroundMuted }]}>
                Necesitamos acceso a la cámara para leer el QR de la tarjeta.
              </Text>

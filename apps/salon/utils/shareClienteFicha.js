...
/**
 * Comparte ficha en texto + JSON; si hay photo_url pública, también el archivo de imagen.
 */
  ...
  if (photoUrl.startsWith('http')) {
    ...
    try {
      ...
      await Sharing.shareAsync(photoPath, {
        ...
        dialogTitle: 'Foto del cliente',
      });
    } catch {
      /* si falla la descarga, seguimos con texto */
    }
  }

  await Sharing.shareAsync(jsonPath, {
    mimeType: 'application/json',
    dialogTitle: 'Ficha cliente (JSON)',
  });
  await Sharing.shareAsync(txtPath, {
    mimeType: 'text/plain',
    dialogTitle: 'Ficha cliente (texto)',
  });
}

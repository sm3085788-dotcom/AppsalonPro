...
/** Fila completa con tinte suave (Tienda / Servicios) */
const ITEM_ROW_TINT = {
  tienda: {
    light: 'rgba(46, 173, 138, 0.10)',
    dark:  'rgba(74, 201, 164, 0.12)',
  },
  citas: {
    light: 'rgba(176, 78, 98, 0.10)',
    dark:  'rgba(200, 120, 138, 0.12)',
  },
};

function resolveRowBackground(itemId, palette, isDark) {
  const tint = ITEM_ROW_TINT[itemId]?.[isDark ? 'dark' : 'light'];
  return tint ?? palette.row;
}
...
function MenuItem({ item, isLast, palette, isDark, fillHeight = false }) {
  ...
  return (
    ...
    <TouchableOpacity
      ...
      style={[
        ...
        { backgroundColor: resolveRowBackground(item.id, palette, isDark) },
